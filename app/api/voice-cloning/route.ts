...
export async function POST(req: NextRequest) {
  try {
    ...
    const enhanceAudioQuality = (formData.get('enhance_audio_quality') as string) || 'true';
    ...
    if (!modelTitle.trim()) {
      return NextResponse.json({ error: "모델 이름(title)이 필요합니다." }, { status: 400 });
    }
    ...
    const response = await fetch('https://api.fish.audio/model', { // 경로가 /v1/models가 아니라 /model이네요!
      method: 'POST',
      headers: {
